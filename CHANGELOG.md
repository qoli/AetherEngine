...
- **Unified sessions now expose real video-output evidence for black-output detection.** `videoOutputSnapshot` publishes route-neutral expected-video, `presented|notExpected|missing`, a session-monotonic frame sequence, generation, latest presented media time, active route, and canonical codec without retaining pixels or request data. During a caller-bounded `waitForVideoFrame(after:timeout:)`, Native polls the exact `AVPlayerItemVideoOutput` for newly decoded output appropriate for display (not a physical AVKit compositor callback), while Hybrid uses its displayed surface/metrics; ordinary Native playback has no periodic pixel-copy cost. Media-clock progress, `readyToPlay`, route state, and Hybrid enqueue counts cannot advance the sequence; seeks, item rebuilds, and route recovery require fresh generation-bound evidence.
- **The unified playback session now owns recoverable control intent and track selection.** `seek(to:)` returns `.applied` or `.superseded`, rapid seeks retain only the newest target, and play/pause/rate updates made during preparation or recovery are applied to the committed route. Public audio/subtitle inventories and Aether-owned selection APIs preserve canonical source-track identity across same-route rebuilds and Native/Hybrid transitions; external subtitles supplied in `LoadOptions` remain part of the canonical request, and an unavailable optional subtitle is published as a capability loss without discarding the saved intent. Capabilities now publish route, range/Dolby Vision, variant bitrate, Atmos, analysis and system-output state, while terminal reports retain the first failure, final failure and exhaustion reason.
- **Progressive unified playback now probes codec facts before route admission.** An ISO-BMFF `ftyp` signature proves only the container and always continues through the same FFmpeg source probe as other progressive media; it no longer creates an early Native trial. Positive H.264 may use the existing AetherEngine-to-loopback-HLS-fMP4 path on the stable player, while positive HEVC enters Hybrid directly. Only an initial classification operation that itself remains transient or inconclusive may create an unknown provisional Native route; recovery re-resolution is fail-closed and never returns HEVC to Native.
- **Unified bounded playback recovery.** `AetherPlaybackSessionFactory.makeSeekableURLVOD` now returns one stable `AetherPlaybackSession` instead of exposing a Native/Hybrid/unsupported launch-result switch to the host. The session retains one `AVPlayer` and outer presentation container while Aether shares one three-failure transport budget across classification, preflight and graph-bound origin requests (1 s/2 s backoff), rebuilds a failed route once, and permits at most one fresh-evidence Native/Hybrid transition per 30-second episode. Initial canonical resolution plus route preparation has a 15-second production deadline; every later recovery episode is capped at 30 seconds, and each suspended operation is clamped to the remaining applicable window. Expiry publishes the typed privacy-safe domain `AetherPlaybackOperationDeadline` and case code `operation.deadlineExceeded`, retires the losing route transaction, and prevents a late completion from taking ownership from its successor. These budgets apply only to Aether-owned recovery of the same request. HLS inspection checks at most three segments, may select one lower capability-compatible variant from the same master, seals stale graphs before fresh preflight, and treats disk-cache loss as a cache capability failure rather than a media terminal. Eligible HEVC SDR/HDR10/HLG can make one hardware-to-software decoder recovery after a hardware rebuild; Dolby Vision remains hardware-only. Authentication, DRM/security/origin violations, corrupt media, presentation drift and required track/system-feature loss remain terminal. Recovery history preserves the first error, attempt outcomes and capability deltas without retaining URLs, headers or credentials.
- **Hybrid carrier real-video decode generations.** The shared VOD demux fanout can now feed a generation-aware video sink alongside the real-audio/black-video AVPlayer carrier, emitting `DecodedVideoFrame` values with PTS, duration, HDR classification, geometry, SAR, rotation, and seek generation. Compressed packets remain in a bounded queue until the host advances AVPlayer-clock demand; startup and explicit seeks require both source coverage through the requested segment and a target-intersecting decoded frame before that carrier segment becomes ready. Video-only sources now use the approved black-video carrier without inventing a silent audio rendition, and black-video segment requests advance their real-video demux directly. HEVC decode starts on VideoToolbox, software codecs use an explicit bounded-latency threading mode, asynchronous decoder work is synchronized at the first-frame gate, and invalid timestamps, stream drift, renderer stalls, decode failures, or invalid clock demand remain typed instead of dropping packets or selecting another media source.
- **Engine-owned hybrid playback session foundation.** `HybridPlaybackSession` composes the black-carrier AVPlayer transport, generation-aware real-video decoder, `AetherHybridPresentationView`, presentation readiness gate, and AVPlayer-clock decode demand without running FFmpeg work on the main actor. Its only Hybrid real-video backend is `AVSampleBufferDisplayLayer`, bound to the exact carrier `AVPlayerItem.timebase`; explicit seeks, observed time jumps, carrier stalls and media-selection changes rebuild one decoder/presentation generation. Missing or replaced carrier clocks, sample-buffer timing/metadata failures, provider/session generation divergence, decoder failure and readiness timeout remain typed failures consumed by the unified bounded-recovery coordinator; no alternate renderer is selected.
- **Hybrid source-axis normalization.** Real-video packet demand, source-coverage checks, decoded-frame PTS, readiness, and sample-buffer presentation now share the carrier's zero-based presentation axis even when the container or video stream starts at a non-zero timestamp. Fresh seek generations preserve absolute timestamps; sources that reset near their origin are rebased only when a distinct packet byte position proves the packet is not the true origin and the fresh demux keyframe index supplies the actual decode anchor. Missing index/position evidence, invalid packet time bases, timestamp overflow, or ambiguous resets fail explicitly instead of guessing a segment-start offset.

### Fixed

- **Unified transport intent can no longer park a ready route at rate zero after a paused seek.** `AetherPlaybackSession` now applies every positive-rate command through route `play()` first and adds `setRate` only for non-1x speed, with command and route generations preventing late seek/rebuild work from replaying stale intent. A ready item that remains explicitly paused receives one same-route reassert after one second; all Native, Audio Bridge, and Hybrid routes share one 30-second real-media-time startup watchdog. Its typed `transportIntentNotApplied` and `startupNoProgress` failures permit one same-route rebuild inside the existing 30-second same-source recovery episode, then terminate without a route transition. The public privacy-safe `transportSnapshot` exposes closed transport/item/waiting phases, generation, rate, media time, range count, recovery sequence and reassert evidence without URLs or request data. `AetherPlaybackRecoveryBudget.maximumStartupOutcomeSeconds` publishes the 60.25-second engine-owned observation/recovery/publication window so a host cannot terminate first.
- **FLV/H.264 now reaches the existing Aether-owned Native HLS-fMP4 remux instead of failing preflight.** The progressive probe now retains FFmpeg's positive `flv` container identity as `flashVideo`; only that exact container fact admits the same H.264 remux contract already used for Matroska, ISO-BMFF, and MPEG-TS. Unknown/other containers remain closed, and HEVC in FLV still enters Hybrid before container admission is considered.
- **Interlaced H.264 now enters unified Hybrid with positive scan evidence.** `SourceProbe` retains FFmpeg field-order as `unknown|progressive|interlaced`; progressive sources and clear, inspected HLS segments carry it into preflight, which routes positively interlaced H.264 to `hybridInterlacedH264` so the Hybrid software decoder runs the existing bwdif/yadif path. Protected interlaced HLS is typed unsupported because Hybrid cannot consume protected packets. The scan fact is pinned across fresh generations, and a fresh positive scan may complete an earlier truly inconclusive provisional Native classification; committed drift remains terminal rather than a hidden backend switch. Forged `unclassifiedURL` profiles with positive codec/container/scan facts are rejected, while progressive or unknown-scan H.264 keeps the existing Native contract.
- **Progressive Hybrid no longer fails near EOF when an Opus bridge meets a sub-frame carrier tail.** A finite Matroska source can report a container duration a few milliseconds beyond its playable A/V samples (for example an ASS cue producing an 8 ms final carrier segment). Because bridged EAC3 access units are indivisible, no packet may start inside that tiny tail even though the real terminal packet ends within one encoded frame of it; the fanout previously raised emptySegment while AVPlayer prefetched the end. The writer now holds only that near-tail real packet until bridge EOF proves it is terminal, then places it in the exact final fragment without synthesizing silence, duplicating source data, or relaxing any earlier/full-segment coverage check. Nested failures also publish the stable closed Hybrid case progressive.audioMuxer.emptySegment with domain AetherEngine.BlackCarrierAudioRenditionMuxer and code 13, rather than compiler-derived wrapper ordinals.
- **Ogg/Vorbis audio-only no longer enters a Native route that AVFoundation cannot play.** Exact finite, seekable progressive/custom media with a positively absent video stream and exactly Vorbis audio now enters a first-class `hybridAudioBridge` execution. The existing demux fanout and `AudioBridge` produce the same source audio in Aether's black-video carrier, while Hybrid explicitly disables source-video decode/readiness and publishes `videoExpected=false`, `notExpected`, codec `none`; startup and Seek retain the one outer `AVPlayer` and source identity. A missing carrier capability or fresh codec/inventory drift stays typed rather than selecting another source, player, or hidden fallback.
- **ASF/WMV dependency gaps are now reported as capability evidence rather than corrupt media.** Source classification recognizes only the positive ASF Header Object GUID, then checks whether the exact linked libavformat provides its `asf` input demuxer. A missing demuxer terminates with `unsupportedCapability` and the closed, privacy-safe case code `dependency.libavformat.asfDemuxerUnavailable`; it no longer becomes the misleading `demux.openFailed` / malformed-media result produced by FFmpeg's generic invalid-data code. The original source, credentials, and route semantics remain unchanged, and the case stays blocked rather than being represented as playable. If a future exact dependency supplies ASF, the same source follows the normal progressive probe with no alternate parser or hidden fallback.
- **Mixed-codec HLS and late item replacement can no longer leave HEVC on Native.** When an uninspected alternate master variant advertises HEVC, Aether now binds the positively inspected same-source selection and rendition graph to Hybrid rather than giving the root master to AVPlayer for independent selection. A Native replacement item or deferred track inspection that positively exposes HEVC is immediately detached and publishes `native.observedHEVCRequiresHybrid`; the real Native state callback preserves that closed code through unified recovery, so bounded recovery skips every same-Native rebuild and may only enter a freshly admitted same-source Hybrid route or emit one typed terminal failure. No source, credential, media identity, or host-player substitution is added.
- **Verified H.264 Native HLS startup no longer fails on advisory early AVAsset metadata.** The unified Native adapter now distinguishes ordinary direct assets from (1) clear direct HLS whose selected segment positively established `nativeHLSContractVerified`, and (2) Aether's loopback HLS-fMP4 item bound to an exact prepared progressive source. In those two cases only, false/failed `asset.load(.isPlayable)` and an empty `asset.loadTracks(.video)` are non-terminal while AVPlayer parses the playlist/init segment; typed item failure plus real media and presented-frame progress remain authoritative. Direct progressive assets, unknown/provisional video, protected or uninspected HLS, and every HEVC fact stay fail-closed. No source, candidate, player, or route substitution is introduced.
- **Late Hybrid displayed-metrics batches now use completion-side carrier time.** An asynchronous `videoPerformanceMetrics` result is mapped to the current generation's enqueued PTS using the exact bound carrier item time re-read at completion, rather than the stale request-start clock. A completion whose carrier time moves backward within the same generation is ignored fail-closed. This remains renderer presence/timing evidence, not luminance analysis or proof that pixels reached a physical panel.
- **Hybrid HEVC B-frames now reach the sample-buffer renderer in truthful display order.** VideoToolbox may return decoded frames in packet/decode order (for example `0, 0.1667, 0.1, 0.0333…`), which previously tripped the Hybrid non-regressing-PTS invariant during preparation. Hybrid now retains exactly the source-declared `AVCodecParameters.video_delay`, emits every unchanged frame and PTS in presentation order through one serialized delivery owner, and marks target readiness only after a frame actually leaves that reorder window. EOF drains every retained frame; a seek/rebuild retires only the prior generation after VideoToolbox's delayed/asynchronous callback barrier. An invalid depth or a regression beyond the declared window remains a typed failure instead of being dropped or hidden.
- **Explicit Hybrid teardown no longer records its interrupted demux read as the media failure.** Closing a black-carrier provider deliberately aborts an in-flight `av_read_frame`, which returns `-1` rather than FFmpeg EOF. Terminal recording is now linearized with provider close: an active-session read error still wins and remains typed, while an abort caused by the close itself cannot overwrite the real terminal evidence or manufacture a second failure.
- **Hybrid completion and post-end Seek now preserve one truthful transport state.** A carrier `didPlayToEnd` is accepted only for the exact current item while its finite clock is within 250 ms of the Aether timeline end, so a delayed pre-Seek notification cannot terminate a new generation parked elsewhere. Accepted completion pauses the carrier and emits one generation-bound ended state/event. A later explicit Seek restores the last explicit play/pause intent and positive rate; the unified session reasserts that intent and derives `.playing` from the real AVPlayer rate/time-control status instead of reporting playback over a rate-zero carrier.
- **Positive HEVC can no longer be misrouted to Native or reported as audio-only.** Unified preflight sends clear, finite, seekable, segment-verified HEVC directly to Hybrid and reports manifest/segment mismatch, uninspected or protected media, unverified Dolby Vision Profile 8.4, and Hybrid capability gaps as typed unsupported. Exact progressive demux stream presence is authoritative: only a positively inspected source with no video stream may use Native audio-only, unidentified video remains video, and any later video track on that audio-only item is typed source divergence. Positive codec evidence always implies video, so contradictory `no video + HEVC` facts still require Hybrid. A provisional Native item that later exposes HEVC fails with typed route evidence and re-resolves through Aether; source-positive video with an empty item track list remains `expected/missing`. Only a positively verified H.264 Native HLS or exact prepared-source H.264 remux may defer the transient empty-AVAsset observation; HEVC and provisional/unknown video fail preparation. Initial/uncommitted recovery no longer reports a capability delta against route-neutral defaults.
- **Progressive Hybrid admission now preserves the exact source facts that selected the route.** `SourceProbe` carries positive demuxer seekability, so a finite forward-only source cannot enter the seek-dependent Hybrid path. The preflight-owned demuxer transfers directly into the first Hybrid generation instead of reopening the URL, and codec, container, duration, seekability, video format, and Dolby Vision configuration are pinned across fresh generations. Software-decoder recovery obtains a fresh one-shot prepared owner and revalidates those committed facts (or the exact HLS resource identity) instead of double-consuming an earlier owner. HEVC-to-H.264 or other source-fact drift is a typed terminal invariant failure and cannot recover into Native.
- **Slow direct Native startup now waits for actual media progress instead of a three-second item-status snapshot.** Once AVFoundation positively accepts the canonical asset, an `.unknown` item is allowed to begin loading under AVPlayer; explicit `.failed` remains immediate, while a privacy-safe 30-second no-progress gate measures media time and starts the existing bounded same-route recovery when necessary. This lets slow HLS items that become ready after 10–15 seconds play normally without turning `playing`, buffered bytes, or `readyToPlay` alone into success. Cross-origin CDN redirects may retain the provider's standard `Referer` and `User-Agent`; `Authorization`, Cookie, and other credential-scoped headers still fail closed at an origin change.
- **Verified H.264 HLS no longer regresses into REMUX or Hybrid.** Clear H.264/AAC MPEG-TS or fMP4 HLS goes directly to AVPlayer after positive segment inspection even when a direct media playlist omits `CODECS` or the manifest declaration is stale. Progressive Matroska H.264 keeps the existing AetherEngine-to-loopback-HLS remux and never passes the raw container URL to AVPlayer; unified positive HEVC is intentionally Hybrid. Native remux Seek now waits for the engine's seek and pending recovery intent to settle within the shared recovery episode instead of judging the clock at the return instant; Hybrid provider restart and carrier Seek share that deadline. Unsupported or contradictory packaging remains an explicit capability boundary rather than silently creating a remote-HLS normalization pipeline.
- **Hybrid presentation pressure no longer becomes a false malformed-media terminal after Seek.** Decoded frames now enter a bounded high-water/low-water mailbox whose producer pauses at capacity and resumes only after `AVSampleBufferDisplayLayer.requestMediaDataWhenReady` drains accepted frames. Generation flush and stop wake blocked producers without dropping accepted frames. Capacity pressure consumes no recovery budget; only a sustained lack of renderer capacity progress emits typed renderer-stall evidence for a same-route presentation rebuild. Presentation, provider and origin errors retain privacy-safe scope/domain/code evidence so operation retirement, transient transport, capability loss and permanent media/security failures no longer collapse into one terminal latch.
