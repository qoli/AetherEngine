...
- **Progressive Native playback now uses the real HLS-fMP4 pipeline.** The unified session no longer hands a raw MKV/progressive origin URL to `AVURLAsset` merely because its payload is H.264 or HEVC. FFmpeg must positively identify a supported Matroska, ISO-BMFF or MPEG-TS container; the admitted route consumes that same open demuxer, remuxes the elementary streams through Aether's loopback HLS-fMP4 server, and installs the local playlist on the session's stable `AVPlayer`. This avoids a second signed-origin probe/open and preserves the canonical URL/header identity. Unknown or unadmitted containers fail explicitly instead of being guessed from the URL suffix or retried as another media source.
