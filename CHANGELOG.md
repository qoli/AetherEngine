...
- **Hybrid carrier real-video decode generations.** The shared VOD demux fanout can now feed a generation-aware video sink alongside the real-audio/black-video AVPlayer carrier, emitting `DecodedVideoFrame` values with PTS, duration, HDR classification, geometry, SAR, rotation, and seek generation. Compressed packets remain in a bounded queue until the host advances AVPlayer-clock demand; startup and explicit seeks require both source coverage through the requested segment and a target-intersecting decoded frame before that carrier segment becomes ready. Video-only sources now use the approved black-video carrier without inventing a silent audio rendition, and black-video segment requests advance their real-video demux directly. HEVC decode stays on VideoToolbox, software codecs use an explicit bounded-latency threading mode, asynchronous decoder work is synchronized at the first-frame gate, and invalid timestamps, stream drift, queue overflow, decode failures, or invalid clock demand terminate with typed errors instead of dropping packets or selecting another playback route.
- **Engine-owned hybrid playback session foundation.** An internal `HybridPlaybackSession` now composes the black-carrier AVPlayer transport, generation-aware real-video decoder, `AetherMetalPlayerView`, presentation readiness gate, and AVPlayer-clock decode demand without running FFmpeg work on the main actor. Explicit host seeks and observed AVKit time jumps restart one decoder generation, suppress the matching managed `timeJumped` notification, wait for both the carrier landing and target frame, and preserve the transport play/pause intent across superseding seeks. Invalid rates, missing carrier clock, provider/session generation divergence, decoder failure, renderer failure, and readiness timeout are typed terminal failures; no alternate renderer or playback route is selected. The composition remains intentionally internal and SDR-only until HDR/Dolby Vision display policy, public host lifecycle, telemetry, and tvOS device acceptance are complete.
- **Hybrid source-axis normalization.** Real-video packet demand, source-coverage checks, decoded-frame PTS, readiness, and Metal scheduling now share the carrier's zero-based presentation axis even when the container or video stream starts at a non-zero timestamp. Fresh seek generations preserve absolute timestamps; sources that reset near their origin are rebased only when a distinct packet byte position proves the packet is not the true origin and the fresh demux keyframe index supplies the actual decode anchor. Missing index/position evidence, invalid packet time bases, timestamp overflow, or ambiguous resets fail explicitly instead of guessing a segment-start offset.
