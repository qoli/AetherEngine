...
- **Verified native HLS no longer regresses into REMUX or Hybrid.** Clear H.264/AAC MPEG-TS or fMP4 HLS goes directly to AVPlayer after positive segment inspection even when a direct media playlist omits `CODECS` or the manifest declaration is stale. Progressive Matroska H.264/HEVC keeps the existing AetherEngine-to-loopback-HLS remux and never passes the raw container URL to AVPlayer. Native remux Seek now waits for the engine's seek and pending recovery intent to settle within the shared recovery episode instead of judging the clock at the return instant; Hybrid provider restart and carrier Seek share that deadline. Hybrid is reserved for genuine non-AVPlayer codecs, while unsupported native-codec packaging remains an explicit capability boundary rather than silently creating a remote-HLS normalization pipeline.
