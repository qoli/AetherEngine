...
The unified `AetherPlaybackSession` enforces that architecture at admission time. A progressive H.264/HEVC source reaches Native only when FFmpeg positively identifies a supported Matroska, ISO-BMFF, or MPEG-TS container. Its preflight owner transfers the already-open demuxer into `HLSVideoEngine`, so playback does not repeat the origin open between inspection and remux. The stable outer `AVPlayer` receives only the loopback HLS URL. Unknown containers, contradictory source facts, a consumed/mismatched prepared request, or an engine backend that does not produce that same player identity fail explicitly; URL suffixes never supply container evidence.

Remote HLS admission is a separate capability from that progressive remux output. After bounded segment inspection positively identifies AVPlayer-supported H.264/AAC MPEG-TS or fMP4, the canonical remote HLS request is handed directly to AVPlayer. A direct media playlist may omit `CODECS`, and stale manifest codec metadata does not authorize REMUX or Hybrid when segment evidence is positive. Feeding a remote playlist into FFmpeg's HLS demuxer and producing another HLS playlist would be a new HLS-in-HLS-out normalization pipeline, not a reuse of the progressive-container capability; it requires a separate design and acceptance effort. Hybrid remains reserved for a codec AVPlayer cannot decode. Native-codec packaging that is not admitted directly is reported as a capability boundary rather than silently entering Hybrid.
