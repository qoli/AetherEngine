...
AetherEngine has three playback pipelines. The lower-level `load(url:)` API selects the initial evidence-backed capability route: the audio-only path when `LoadOptions.audioOnly` is set, otherwise the native or software video path based on the source's video codec. The formal unified `AetherPlaybackSession` adds a stricter integration policy: positive HEVC always starts and remains on `hybridCarrier`, regardless of AVPlayer's or VideoToolbox's lower-level HEVC capability. ISO-BMFF is only container evidence and must pass the progressive source probe before route admission. That initial route is not an irrevocable lifetime choice for other admitted route pairs. Runtime evidence may trigger a bounded, observable Aether-owned reopen, rebuild, decoder/backend change, or route transition inside the same session, provided it preserves the canonical media request, source/content identity, authorization, DRM meaning, selected track, and provenance and publishes any capability delta. Hosts do not select a second engine or source after an opaque failure, and recovery never sends positive HEVC to Native.

Production unified sessions bound the complete initial preparation window to
15 seconds and each subsequent recovery episode to 30 seconds. Every suspended
Aether-owned operation is clamped to the remaining applicable window. Expiry
is a typed `AetherPlaybackOperationDeadline` failure with case code
`operation.deadlineExceeded`; the route transaction is retired immediately so
a late completion cannot regain item, decoder, display, or route ownership.
The deadline may end or advance same-request Aether recovery, but it never
licenses a host-selected server URL or any other semantic substitution.

For progressive unified playback, audio-only admission requires the exact demux
probe to positively report that no video stream exists. A demux-present video
stream remains video even when its codec or dimensions are unidentified, and an
inconclusive stream inventory is typed unsupported rather than treated as
absence. Ordinary AVPlayer-supported audio enters Native. Exact finite,
seekable Vorbis enters `hybridCarrier`: the one AVPlayer still clocks Aether's
generated black-video carrier while `AudioBridge` decodes the same source audio
into its audio rendition. No real-video decoder/readiness gate exists for that
execution, and its public output remains `videoExpected=false`, `notExpected`,
codec `none`. Positive video codec evidence promotes the item to video. If an
admitted audio-only item later exposes any video track, it contradicts source
identity and terminates with typed divergence. In particular, `HEVC` can never
be neutralized by a contradictory no-video flag or reach Native through this
route.

### Presented-frame evidence

The unified `AetherPlaybackSession` publishes one route-neutral `videoOutputSnapshot`. Its `frameSequence` is monotonic for the outer session even when a seek, item rebuild, decoder change, or route recovery advances `frameGeneration`; the new generation returns to `missing` until its own real frame is observed. The snapshot records only expected-video, `presented|notExpected|missing`, generation/sequence, latest presented media time and observation uptime, active route, and the canonical codec spelling (`h264|hevc|av1|vp9|vp8|mpeg2|mpeg4|vc1|unknown|none`). It contains no source URL, headers, credentials, cookies, tokens, search terms, or pixel data.

Native observation attaches an `AVPlayerItemVideoOutput` with `suppressesPlayerRendering = false` to the exact current item. Only an active caller-bounded `waitForVideoFrame` window polls it; normal production playback performs no periodic pixel copying. A sequence advances only when a host-time poll copies a new AVFoundation-decoded pixel buffer appropriate for display and its media time corresponds to the current player clock. This proves that AVFoundation produced displayable decoded output, not that a physical AVKit compositor callback reached the panel; item readiness and a moving AVPlayer clock do not count. Hybrid observation is stronger presentation evidence and is generation-bound to `AVSampleBufferVideoRenderer`: on the view's first, paused generation it can prove the displayed pixel buffer directly, and while running or after a seek it requires an increase in `totalNumberOfFrames - numberOfDroppedFrames`, mapped to a current-generation enqueued PTS at or behind the carrier clock. Because that metrics read is asynchronous, the PTS bound uses the exact bound carrier item's time re-read when the result completes, never the stale request-start time; a completion whose carrier time regresses within one generation is ignored fail-closed. A seek deliberately preserves the old image, so that unlabelled pixel buffer is never credited to the new generation. Enqueue count alone never counts. Platforms without either truthful Hybrid displayed surface remain `missing` rather than guessing success.

`waitForVideoFrame(after:timeout:)` actively polls those Aether-owned outputs until it observes a session sequence newer than the caller's baseline, returns immediately for a positively observed no-video item, and otherwise returns the latest fail-closed snapshot at the caller's deadline. Acceptance checks the authoritative fixture expectation separately: `notExpected` cannot excuse a case that was required to contain video. No code reads luminance or classifies image brightness; the contract detects missing renderer output, not artistic black frames, display-pipeline luminance, or whether pixels reached a physical panel.

### Native AVPlayer pipeline (lower-level capability; unified H.264 route)

Demux the source with libavformat, re-mux the elementary streams on the fly into HLS-fMP4, serve them from a local HTTP server on `127.0.0.1:<port>`, point `AVPlayer` at the playlist. Apple's stack does all decode, all HDR / Dolby Vision signaling over HDMI, all audio routing. The lower-level engine retains HEVC capability here, but that implementation fact is not unified-route permission. `AetherPlaybackSession` uses this path for admitted progressive H.264; positively interlaced H.264 enters Hybrid so its software decoder can deinterlace, and positive HEVC uses Hybrid from initial preflight.
...
The unified `AetherPlaybackSession` enforces that architecture at admission time. A progressive H.264 source reaches Native only after FFmpeg positively identifies a supported Matroska, ISO-BMFF, MPEG-TS, or FLV container. FLV is admitted by the exact input-format name, never its suffix or MIME type, and AVPlayer receives Aether's loopback HLS-fMP4 rather than raw FLV. Positive interlaced field order is a stricter boundary: progressive sources and clear inspected HLS segments enter Hybrid and its software decoder/deinterlacer, while progressive or unknown scan evidence retains Native admission. Protected interlaced HLS is typed unsupported rather than handed to Native or stripped of protection. Its preflight owner transfers the already-open demuxer into the selected route, so playback does not repeat the origin open between inspection and construction. Once an exact Native owner has been consumed, the source profile revalidated, and Aether's loopback item installed on the stable player, early false/failed `asset.load(.isPlayable)` and empty `asset.loadTracks(.video)` observations are advisory: generated HLS may not expose asset tracks until AVPlayer parses the playlist and init segment, so actual engine/item failure and real media/video-output progress remain authoritative. The same bounded advisory applies to clear direct H.264 HLS only after selected-segment inspection produced `nativeHLSContractVerified` with known packaging; it does not apply to protected/manifest-only HLS. Ordinary direct assets, unclassified/provisional video, and unknown codec facts retain hard metadata gates. A provisional Native route is valid only while codec, scan, container, and stream presence all remain inconclusive; a later positive HEVC, interlaced-H.264, or audio-only Vorbis classification may make one bounded same-request Hybrid transition, while a forged positive provisional profile is typed unsupported. HEVC never receives either Native advisory and cannot enter Native. A progressive HEVC source instead enters Hybrid only when that exact demuxer establishes finite, non-live, positively seekable media and supported format facts; finite duration is not seekability, and a container signature alone can never create a provisional Native trial. Hybrid consumes the same prepared demuxer for its first generation, then pins codec, container, scan type, duration, seekability, video format, and Dolby Vision configuration across fresh generations. Decoder/backend recovery fresh-resolves a new one-shot owner and revalidates those facts (or the exact HLS resource identity) before construction; it never double-consumes the owner used by an earlier build. Any drift is typed source-identity failure rather than permission to reinterpret the URL or change route. Unknown containers for H.264, contradictory source facts, a consumed/mismatched prepared request, or an engine backend that does not produce the stable player identity fail explicitly; URL suffixes never supply container or route evidence.

Remote HLS admission is a separate capability from that progressive remux output. After bounded segment inspection positively identifies supported progressive-scan H.264/AAC MPEG-TS or fMP4, the canonical remote HLS request is handed directly to AVPlayer. A direct media playlist may omit `CODECS`; positive H.264 segment evidence remains authoritative, while positive interlaced field-order instead binds the inspected clear VOD graph to Hybrid. Clear HEVC requires a finite seekable VOD, verified segment codec (including manifest-missing but segment-verified evidence), and complete Hybrid capability, then enters `hybridCarrier` even for `hvc1`. HEVC manifest/segment mismatch, uninspected segments, content protection/DRM, protected interlaced H.264, unverified Dolby Vision Profile 8.4 facts, or missing Hybrid capability is typed unsupported. Feeding a remote playlist into FFmpeg's HLS demuxer and producing another HLS playlist would be a new HLS-in-HLS-out normalization pipeline, not a reuse of the progressive-container capability; it requires a separate design and acceptance effort.

A root master is not equivalent to that inspected direct media playlist. If a
different, uninspected variant advertises HEVC, handing the root to Native would
let AVPlayer select media Aether never inspected. The selected H.264 resource
and rendition graph is therefore bound to Hybrid instead, preserving the same
request without a second source or guessed variant. A later Native item
replacement or deferred track load that positively exposes HEVC immediately
detaches the item and emits `native.observedHEVCRequiresHybrid`; bounded recovery
may only fresh-resolve the same request into Hybrid or terminate, never rebuild
Native.

Seek ownership stays in the selected engine. For progressive Native remux, the unified adapter waits for `AetherEngine.seek`, `isSeeking`, and pending recovery intent to settle, including cache hits, `RestartCoalescer`, fresh-demux reopen, wedge reconcile/re-anchor, and item revive. It only bounds that existing operation by the remaining 30-second recovery episode and reports `.superseded` when a newer seek wins; it does not infer failure from an instantaneous `currentTime` delta. Hybrid provider restart, carrier Seek, target-generation preparation, and presentation readiness consume one shared Seek deadline; a timed-out step resolves the adapter wait with a typed failure for the recovery coordinator instead of suspending the host indefinitely. Readiness and transport flags are operational signals; acceptance still requires demonstrated media-time progress. Rapid Hybrid Seek coalescing is accepted for the graph-bound HLS provider: one target wins, intermediate operations resolve `.superseded`, generation growth stays bounded, and the carrier clock must land and continue advancing. Other provider/source contracts require their own physical fixture rather than inheriting that result.

Hybrid completion is also engine-owned state, not a host inference. The exact current carrier item's `didPlayToEnd` notification is accepted only while the real finite carrier clock is within a bounded 250 ms window of the Aether timeline end; the notification or a near-end clock alone is insufficient. Accepted completion pauses the carrier, closes decode demand, publishes one `.ended(generation:)` Hybrid state and one privacy-safe `playbackCompleted` telemetry event, and the unified `AetherPlaybackSession` exposes `.ended`. A later explicit Seek clears the completion latch and establishes the next ready generation. A delayed pre-Seek end notification is rejected against the new carrier clock. The seek restores the last explicit play/pause intent and positive rate, and the outer session reasserts that intent before publishing a state derived from the real `AVPlayer` rate/time-control status, so `.playing` cannot describe a carrier still parked at rate zero. A route state or host dismissal must never impersonate completion.
...
The unified Hybrid route also owns every positive HEVC request and can use the
existing VideoToolbox HEVC decoder behind its sample-buffer presentation
contract. This does not turn the legacy software-host pipeline into the unified
route; it documents reuse of the decoder capability under Hybrid ownership.
...
AV1+DV (Profile 10.0 / 10.1 / 10.4) initially routes through the native path on hardware-AV1 hosts via the `dav1` / `av01` track type plus the source's `dvvC` box. AV1+Atmos is genuinely rare in the wild (mastering still runs in HEVC overwhelmingly), so the SW pipeline's lack of Atmos passthrough is a theoretical limitation rather than a real one. Initial dispatch happens at load time; an evidence-backed Aether-owned recovery may later change the implementation while preserving the same request and explicitly reporting any capability change. Hosts see a unified `@Published` state surface either way.
...
│   └── WebVTTBuilder.swift                  Builds plain-text WebVTT on the AVPlayer timeline for native carrier SUBTITLES renditions. Graph-bound HLS republishes admitted WebVTT bytes; the progressive Hybrid pump decodes only faithfully convertible SubRip/mov_text/WebVTT/text packets into the same carrier contract. Styled and bitmap tracks never enter this lossy path.
├── Video/
...
| --- | --- | --- |
| [qoli/FFmpegBuild](https://github.com/qoli/FFmpegBuild) | LGPL-3.0 build package; independent bundled licenses | Exact-revision Gate 1A fork of the slim FFmpeg 8.1 build (avcodec / avformat / avutil / swresample / swscale / avfilter + zimg) for demux + HLS-fMP4 mux + AudioBridge FLAC encode + SW-path dav1d decode + sws_scale YUV → NV12 / P010. It publishes exact source/rebuild/relink artifacts and bundles libzvbi for the DVB teletext decoder (#107); static App Store legal review remains open. |
| [LibDovi](https://github.com/superuser404notfound/LibDovi) | MIT at bundled libdovi 3.3.2 tag | libdovi (the `dolby_vision` crate's C API) for live Dolby Vision Profile 7 to single-layer 8.1 RPU conversion (`dovi_convert_rpu_with_mode`, mode 2), so the Apple TV engages real DV on dual-layer UHD-BD remuxes instead of plain HDR10. Prebuilt xcframework, no Rust at the consumer's build time |
| [qoli/swift-libass](https://github.com/qoli/swift-libass) | MIT wrapper; transitive licenses include ISC, LGPL-2.1-or-later, FreeType License, Old MIT and libpng terms | Exact-revision Gate 1A fork used only by the Aether-owned Hybrid styled-subtitle overlay. It exact-pins ffmpeg-kit and every native source, publishes rebuilt XCFrameworks and a source/relink bundle. Aether owns the carrier-clock sampling, lifecycle, blending, track-local failure policy and AVKit menu contract. See [`THIRD_PARTY_NOTICES.md`](../THIRD_PARTY_NOTICES.md) for exact revisions, archive hashes and remaining legal/relink requirements. |
